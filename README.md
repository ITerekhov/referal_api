...
а также существует база данных {db_name} *
...
указать токен в формате: "Token" {token}
...
-	запрос PUT позволяет обновить номер телефона (если указанный номер ещё не регистрировался),
	и указать инвайт-код(если он не был указан при регистрации). Данный указываем в формате:
{	"user": {
        "phone": new_phone,}}


* Все указанные переменные содержатся в файле config.py в директории проекта
